...
export const Login = props => {
    ...
    const executaLogin = async evento => {
        try{
            ...
            const resultado = await request('login', 'POST', body);
            if(resultado?.data?.token){
                localStorage.setItem('accessToken', resultado.data.token);
                localStorage.setItem('usuarioNome', resultado.data.user_name);
                localStorage.setItem('usuarioEmail', resultado.data.user_email);
